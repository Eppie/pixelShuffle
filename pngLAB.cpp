...
double totalDiff( Color** src, Color** dst ) {
	double totalDiff = 0;
	Color* rowSrc;
	Color* rowDst;
	Color* sPx;
...
void processPNGFile( Color** src, Color** dst ) {
	...
	for( int l = 0; l < 1; l++ ) {
		for( int j = 0; j < orderedLoopCount; j++ ) {
			...
			stringstream out;
			out.imbue( locale( "" ) );
			out << "Iteration #" << j + ( ( orderedLoopCount + randomLoopCount ) * l )
				<< ", Diff: " << fixed << t
				<< ", Swaps: " << numSwaps << " ( " << ( numSwaps / static_cast<double>( innerOrderedLoopCount ) * 100.0 ) << "% )"
				<< " (ordered)" << endl;
			cout << out.str();
#endif // OUTPUT
#ifdef ANIMATION
		...
		for( int j = 0; j < randomLoopCount; j++ ) {
			...
			stringstream out;
			out.imbue( locale( "" ) );
			out << "Iteration #" << j + ( ( orderedLoopCount + randomLoopCount ) * l ) + orderedLoopCount
				<< ", Diff: " << fixed << t
				<< ", Swaps: " << numSwaps << " ( " << ( numSwaps / static_cast<double>( j * 1e5 ) * 100.0 ) << "% )"
				<< " (random)" << endl;
			cout << out.str();
#endif // OUTPUT
#ifdef ANIMATION
