...
constexpr uint64_t kInitialRandomSeed = 0xE9D3F10A51A11CE5ULL;
...
std::vector<float> salienceForTarget( const Geometry& geometry, const std::vector<LabColor>& target ) {
	std::vector<float> salience( geometry.pixelCount(), 0.0f );
	float maxValue = 0.0f;
	for( int y = 0; y < geometry.height; ++y ) {
		for( int x = 0; x < geometry.width; ++x ) {
			const size_t index = static_cast<size_t>( y ) * static_cast<size_t>( geometry.width ) + static_cast<size_t>( x );
			const int left = std::max( 0, x - 1 );
			const int right = std::min( geometry.width - 1, x + 1 );
			const int top = std::max( 0, y - 1 );
			const int bottom = std::min( geometry.height - 1, y + 1 );
			const size_t leftIndex = static_cast<size_t>( y ) * static_cast<size_t>( geometry.width ) + static_cast<size_t>( left );
			const size_t rightIndex = static_cast<size_t>( y ) * static_cast<size_t>( geometry.width ) + static_cast<size_t>( right );
			const size_t topIndex = static_cast<size_t>( top ) * static_cast<size_t>( geometry.width ) + static_cast<size_t>( x );
			const size_t bottomIndex = static_cast<size_t>( bottom ) * static_cast<size_t>( geometry.width ) + static_cast<size_t>( x );
			const float horizontal = std::sqrt( squaredLabDistance( target[leftIndex], target[rightIndex] ) );
			const float vertical = std::sqrt( squaredLabDistance( target[topIndex], target[bottomIndex] ) );
			const float chroma = std::sqrt( ( target[index].a * target[index].a ) + ( target[index].b * target[index].b ) );
			const float value = horizontal + vertical + ( chroma * 0.035f );
			salience[index] = value;
	...
	for( float& value : salience ) {
		value = std::sqrt( value / maxValue );
	}
	return salience;
}

std::vector<float> wavefrontDelayForTarget( const Geometry& geometry, const std::vector<float>& salience ) {
	std::vector<float> order( geometry.pixelCount(), 0.0f );
	std::vector<size_t> ids( geometry.pixelCount() );
	std::iota( ids.begin(), ids.end(), 0u );
	const float cx = ( static_cast<float>( geometry.width ) - 1.0f ) * 0.5f;
	const float cy = ( static_cast<float>( geometry.height ) - 1.0f ) * 0.5f;
	std::sort( ids.begin(), ids.end(), [&]( const size_t lhs, const size_t rhs ) {
		const float lx = static_cast<float>( lhs % static_cast<size_t>( geometry.width ) ) - cx;
		const float ly = static_cast<float>( lhs / static_cast<size_t>( geometry.width ) ) - cy;
		const float rx = static_cast<float>( rhs % static_cast<size_t>( geometry.width ) ) - cx;
		const float ry = static_cast<float>( rhs / static_cast<size_t>( geometry.width ) ) - cy;
		const float leftDistance = std::sqrt( ( lx * lx ) + ( ly * ly ) );
		const float rightDistance = std::sqrt( ( rx * rx ) + ( ry * ry ) );
		const float leftScore = salience[lhs] * 1.25f - leftDistance * 0.0009f + hashUnit( lhs ) * 0.015f;
		const float rightScore = salience[rhs] * 1.25f - rightDistance * 0.0009f + hashUnit( rhs ) * 0.015f;
		return leftScore > rightScore;
	} );

	const float invMax = ids.size() > 1u ? 1.0f / static_cast<float>( ids.size() - 1u ) : 1.0f;
	for( size_t rank = 0; rank < ids.size(); ++rank ) {
		const float ranked = static_cast<float>( rank ) * invMax;
		order[ids[rank]] = std::pow( ranked, 0.78f );
	}
	return order;
}
...
void renderFlowFrame(
	...
	const std::vector<float>& destinationDelay,
	const std::vector<float>& destinationSalience,
	...
	const float cx = ( static_cast<float>( geometry.width ) - 1.0f ) * 0.5f;
	const float cy = ( static_cast<float>( geometry.height ) - 1.0f ) * 0.5f;
	const float backgroundWeight = 1.15f;

	for( size_t i = 0; i < count; ++i ) {
		const Pixel& finalPixel = pixels[finalAssignment[i]];
		const float reveal = smoothstep( ( globalProgress - ( destinationDelay[i] * 0.48f ) ) / 0.42f );
		const float brightness = 0.16f + ( 0.62f * reveal ) + ( 0.18f * destinationSalience[i] * reveal );
	...
	for( size_t pixelId = 0; pixelId < count; ++pixelId ) {
		...
		const float delay = 0.55f * destinationDelay[dstIndex] + 0.05f * hashUnit( pixelId );
		const float rawProgress = ( globalProgress - delay ) / std::max( 0.08f, 1.0f - delay );
		const float launch = smoothstep( ( rawProgress - 0.04f ) / 0.22f );
		if( launch <= 0.0f ) {
			continue;
		}
		const float p = smoothstep( rawProgress );
		...
		const float salient = destinationSalience[dstIndex];
		const float swirl = std::min( 54.0f, 5.0f + ( distance * ( 0.12f + salient * 0.08f ) ) ) * arc * sign;
		const float radial = ( 0.02f + salient * 0.03f ) * arc;
		...
		const float opacity = 0.15f + ( 1.10f * launch );
		splat( geometry, pixels[pixelId], x, y, opacity, red, green, blue, weight );
	}
...
int main( int argc, char* argv[] ) {
	...
	const std::vector<float> destinationSalience = salienceForTarget( targetGeometry, targetLab );
	const std::vector<float> destinationDelay = wavefrontDelayForTarget( targetGeometry, destinationSalience );
	...
	for( int frameIndex = 0; frameIndex < kFrameCount; ++frameIndex ) {
		renderFlowFrame( targetGeometry, pixels, assignment, destinationForPixel, destinationDelay, destinationSalience, outputRows, frameIndex );
		frames.write();
	}
