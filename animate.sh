#!/bin/bash
# Shared animation pipeline, sourced by run.sh (single pair) and generateAll.sh (batch).
#
# generateImage <variant-suffix> <palette-stem> <target-stem>
#   runs ./png<suffix> on images/<palette>.png + images/<target>.png, pads the frame
#   sequence (hold start / hold result / show palette), encodes a webm, shrinks the
#   result png, and cleans up the intermediate frames.
generateImage() {
	outFileName="${2}TO${3}"
